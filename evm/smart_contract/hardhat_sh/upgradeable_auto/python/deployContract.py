...
def polygon_set_logger(python_pgm):
    ...
    logHandler = logging.handlers.TimedRotatingFileHandler(filename=f'/home/whsoo8232/Projects/MY/my_blockchain_lab/evm/smart_contract/hardhat_sh/upgradeable_auto/logs/{python_pgm}.log', when='midnight', interval=1, encoding='utf-8')
...
def polygon_write_dotEnv(envFile, rpcUrlKey, etherscanKey, ownerPK):
    f = open(envFile, 'w')

    f.write(f"RPC_URL_KEY='{rpcUrlKey}'\n")
    if etherscanKey == "":
...
def polygon_deploy_contract(network, rpcApiKey, etherscanKey, ownerPK, tokenType, targetTokenName, targetSymbolName, targetAmount=None, logger=None, python_pgm=None):
    ...
    for line in lines:
        ...
        if "Transaction hash" in line:
            start = line.index("Transaction hash") + 18
            contractTransactionHash = line[start:]
    abiData = []
    byteCodeData = ""
    ...
    return retCode, retMessage, abiData, contractAddress, contractTransactionHash
